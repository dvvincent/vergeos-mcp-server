...
async function listVMSnapshots(vmId) {
  const vm = await getVM(vmId);
  const machineId = vm.machine;
  const snapshots = await apiRequest(`/api/v4/machine_snapshots?machine=${machineId}&fields=most`);
...
async function createVMSnapshot(vmId, options = {}) {
  ...
  const result = await apiRequest("/api/v4/machine_snapshots", {
    method: "POST",
...
async function deleteVMSnapshot(snapshotId) {
  await apiRequest(`/api/v4/machine_snapshots/${snapshotId}`, { method: "DELETE" });
  return { success: true, message: `Snapshot ${snapshotId} deleted` };
}

async function restoreVMSnapshot(vmId, snapshotId) {
  const result = await apiRequest("/api/v4/vm_actions", {
    method: "POST",
...
async function executeTool(name, args) {
  switch (name) {
    ...
    case "list_vm_snapshots": return listVMSnapshots(args.vm_id);
    case "create_vm_snapshot": return createVMSnapshot(args.vm_id, args);
    case "delete_vm_snapshot": return deleteVMSnapshot(args.snapshot_id);
    case "restore_vm_snapshot": return restoreVMSnapshot(args.vm_id, args.snapshot_id);
